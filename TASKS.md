...
- [x] Add `/llms.txt` check to the audit (score and recommend it)
- [x] Expand `robots.txt` check to detect AI-specific bot directives (`GPTBot`, `ClaudeBot`, `PerplexityBot`, `Googlebot-Extended`)
- [x] Add check for `noai` / `noimageai` meta tags
...
## 4. New Features
- [x] Add PDF export of audit results
- [x] Connect Audit and LLM Optimizer — one-click schema generation from audit results

## 5. Code Quality
- [x] Move Anthropic model name to a named constant or env var
- [x] Create `tests/` folder and add unit tests for audit analysis functions
